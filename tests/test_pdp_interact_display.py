...
def test_pdp_interact_display_binary():
    ...
    with open(output_path, "a") as outbook:
        ...
        titanic_numeric_binary_pdp_str = (
            "pdp_fare_sex = pdp.pdp_interact(model=titanic_model, dataset=titanic_data, "
        ...
        titanic_numeric_binary_default_config = (
            "pdp_interact_out=pdp_fare_sex, feature_names=['Fare', 'Sex']"
        )
        titanic_numeric_binary_add_configs = [
            "plot_type='contour', x_quantile=True, plot_pdp=False",
        ...
        for config in titanic_numeric_binary_add_configs:
            outbook.write(
                PDP_INTERACT_PLOT_STR
                % (titanic_numeric_binary_default_config + ", " + config)
            )
            outbook.write(CODE_GAP_STR)
        ...
        # numeric and binary feature plot config
        titanic_numeric_binary_add_configs = [
            "plot_type='grid', plot_pdp=False, plot_params=plot_params"
        ]
        for config in titanic_numeric_binary_add_configs:
            outbook.write(
                PDP_INTERACT_PLOT_STR
        ...
        titanic_onehot_binary_pdp_str = (
            "pdp_embark_sex = pdp.pdp_interact(model=titanic_model, dataset=titanic_data, "
        ...
        titanic_onehot_binary_default_config = (
            "pdp_interact_out=pdp_embark_sex, feature_names=['Embark', 'Sex']"
        )
        titanic_onehot_binary_add_configs = [
            "plot_type='grid', plot_pdp=False",
            "plot_type='grid', plot_pdp=True",
        ]
        for config in titanic_onehot_binary_add_configs:
            outbook.write(
                PDP_INTERACT_PLOT_STR
                % (titanic_onehot_binary_default_config + ", " + config)
            )
            outbook.write(CODE_GAP_STR)
...
def test_pdp_interact_display_regression():
    ...
    with open(output_path, "a") as outbook:
        ...
        ross_numeric_binary_pdp_str = (
            "pdp_weekofyear_SchoolHoliday = pdp.pdp_interact(model=ross_model, "
        ...
        for config_idx, config in enumerate(ross_numeric_binary_add_configs):
            outbook.write(
                PDP_INTERACT_PLOT_STR
                % (ross_numeric_binary_default_config + ", " + config)
            )
            if config_idx == 0:
                outbook.write(
        ...
        ross_onehot_binary_pdp_str = (
            "pdp_StoreType_SchoolHoliday = pdp.pdp_interact(model=ross_model, "
            "dataset=ross_data, model_features=ross_features, "
        ...
        for config in ross_onehot_binary_add_configs:
            outbook.write(
                PDP_INTERACT_PLOT_STR
                % (ross_onehot_binary_default_config + ", " + config)
            )
            outbook.write(CODE_GAP_STR)
...
def test_pdp_interact_display_multiclass():
    ...
    with open(output_path, "a") as outbook:
        ...
        otto_numeric_numeric_pdp_str = (
            "pdp_67_24_rf = pdp.pdp_interact(model=otto_model, dataset=otto_data, "
        ...
        otto_numeric_numeric_default_config = (
            "pdp_interact_out=pdp_67_24_rf, feature_names=['feat_67', 'feat_24']"
        )
        otto_numeric_numeric_add_configs = [
            "plot_type='contour', x_quantile=False, plot_pdp=False, ncols=3",
        ...
        for config in otto_numeric_numeric_add_configs:
            outbook.write(
                PDP_INTERACT_PLOT_STR
                % (otto_numeric_numeric_default_config + ", " + config)
            )
            outbook.write(CODE_GAP_STR)
