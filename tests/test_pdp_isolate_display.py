...
def test_display_binary():
    ...
    with open(output_path, "a") as outbook:
        ...
        titanic_binary_pdp_str = (
            "pdp_sex = pdp.pdp_isolate(model=titanic_model, dataset=titanic_data, "
        ...
        for config_idx, config in enumerate(titanic_binary_add_configs):
            outbook.write(
                PDP_PLOT_STR % (titanic_binary_default_config + ", " + config)
            )
            if config_idx == 0:
                outbook.write(
        ...
        titanic_onehot_default_config = (
            "pdp_isolate_out=pdp_embark, feature_name='Embark'"
        )
        titanic_onehot_add_configs = [
            "plot_pts_dist=True",
            "plot_pts_dist=True, plot_lines=True, frac_to_plot=0.1",
            "plot_pts_dist=True, plot_lines=True, frac_to_plot=0.1, center=False",
            "plot_pts_dist=True, cluster=True, n_cluster_centers=10, cluster_method='approx'",
        ...
        for config in titanic_onehot_add_configs:
            outbook.write(
                PDP_PLOT_STR % (titanic_onehot_default_config + ", " + config)
            )
            outbook.write(CODE_GAP_STR)
        ...
        titanic_numeric_pdp_str = (
            "pdp_fare = pdp.pdp_isolate(model=titanic_model, dataset=titanic_data, "
        ...
        for config in titanic_numeric_add_configs:
            outbook.write(
                PDP_PLOT_STR % (titanic_numeric_default_config + ", " + config)
            )
            outbook.write(CODE_GAP_STR)
...
def test_display_regression():
    ...
    with open(output_path, "a") as outbook:
        ...
        ross_binary_pdp_str = (
            "pdp_SchoolHoliday = pdp.pdp_isolate(model=ross_model, dataset=ross_data, "
        ...
        # binary feature plot config
        ross_binary_default_config = (
            "pdp_isolate_out=pdp_SchoolHoliday, feature_name='SchoolHoliday'"
        )
        ross_binary_add_configs = [
        ...
        ross_onehot_pdp_str = (
            "pdp_StoreType = pdp.pdp_isolate(model=ross_model, dataset=ross_data, "
        ...
        # onehot feature plot config
        ross_onehot_default_config = (
            "pdp_isolate_out=pdp_StoreType, feature_name='StoreType'"
        )
        ross_onehot_add_configs = [
            "plot_lines=True, frac_to_plot=100",
            "plot_pts_dist=True, plot_lines=True, frac_to_plot=100",
        ]
        ...
        ross_numeric_pdp_str = (
            "pdp_weekofyear = pdp.pdp_isolate(model=ross_model, dataset=ross_data, "
        ...
        # numeric feature plot config
        ross_numeric_default_config = (
            "pdp_isolate_out=pdp_weekofyear, feature_name='weekofyear'"
        )
        ross_numeric_add_configs = [
            "plot_lines=True, frac_to_plot=100",
            "plot_pts_dist=True, plot_lines=True, frac_to_plot=100",
...
def test_display_multiclass():
    ...
    with open(output_path, "a") as outbook:
        ...
        otto_numeric_pdp_str = (
            "pdp_feat_67_rf = pdp.pdp_isolate(model=otto_model, dataset=otto_data, "
        ...
        # numeric feature plot config
        otto_numeric_default_config = (
            "pdp_isolate_out=pdp_feat_67_rf, feature_name='feat_67'"
        )
        otto_numeric_add_configs = [
            "center=True, x_quantile=True, ncols=3",
